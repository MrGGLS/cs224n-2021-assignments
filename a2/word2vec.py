...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    all_pro=softmax(outsideVectors.dot(centerWordVec))
    loss=-np.log(all_pro[outsideWordIdx])
    gradCenterVec=outsideVectors.T.dot(all_pro)-outsideVectors[outsideWordIdx]
    all_pro[outsideWordIdx] -= 1
    gradOutsideVecs=np.expand_dims(all_pro,axis=1)@(np.expand_dims(centerWordVec, axis=0))
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    negSamples=outsideVectors[negSampleWordIndices]
    loss=-np.log(sigmoid(outsideVectors[outsideWordIdx].dot(centerWordVec)))-np.sum(np.log(sigmoid(-negSamples.dot(centerWordVec))))

    gradCenterVec=-(1-sigmoid(outsideVectors[outsideWordIdx].dot(centerWordVec)))*outsideVectors[outsideWordIdx]+np.sum(np.expand_dims((1-sigmoid(-negSamples.dot(centerWordVec))), axis=1)*negSamples, axis=0)
    ...
    for idx, neg_idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_idx]+=(1-sigmoid(-negSamples[idx].dot(centerWordVec)))*centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for o_w in outsideWords:
        res=list(word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[o_w], outsideVectors, dataset))
        loss+=res[0]
        gradCenterVecs[word2Ind[currentCenterWord]]+=res[1]
        gradOutsideVectors+=res[2]
    ### END YOUR CODE
