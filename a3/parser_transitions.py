...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        for idx,transition in enumerate(transitions):
            minibatch[idx].parse([transition])
            if len(minibatch[idx].stack) is 1 and len(minibatch[idx].buffer) is 0:
                unfinished_parses.remove(minibatch[idx])
    dependencies=[parses.dependencies for parses in partial_parses]
    ### END YOUR CODE
    return dependencies
