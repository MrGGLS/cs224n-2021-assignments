...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder=nn.LSTM(self.model_embeddings.embed_size, self.hidden_size, bidirectional=True, bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X=torch.tensor([ self.model_embeddings.source(b).tolist() for b in source_padded])
        packed_X=pack_padded_sequence(X, lengths=source_lengths).to(device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
        enc_hiddens, (h_n, c_n)=self.encoder(packed_X)
        enc_hiddens, _=pad_packed_sequence(enc_hiddens)
        enc_hiddens=enc_hiddens.permute(1, 0, 2) # change dims [src_len b 2*h] -> [b src_len 2*h]
        init_decoder_hidden=self.h_projection(h_n.permute(1, 0, 2).reshape(-1, 2*self.hidden_size))
        init_decoder_cell=self.c_projection(c_n.permute(1, 0, 2).reshape(-1, 2*self.hidden_size))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y=torch.split(Y, 1)
        for Y_t in Y:
            Ybar_t=torch.cat((torch.squeeze(Y_t), o_prev), 1)
