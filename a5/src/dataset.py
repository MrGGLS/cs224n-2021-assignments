...
#         chars = sorted(list(set(data + u"\u2047" + u"\u25A1")))
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated = document[:random.randint(min(4, int(self.block_size * 7 / 8)), max(4, int(self.block_size * 7 / 8)))]
        mask_content_len = min(max(1, int(random.gauss(len(truncated) / 4., len(truncated)/6.))), len(truncated))
        mask_content_start_idx = int(random.randint(0, len(truncated) - mask_content_len))

        prefix = truncated[:mask_content_start_idx]
        mask_content = truncated[mask_content_start_idx:(mask_content_start_idx + mask_content_len)]
        suffix = truncated[(mask_content_start_idx + mask_content_len):]

        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + mask_content
        masked_string = masked_string+(self.block_size-len(masked_string))*self.PAD_CHAR
