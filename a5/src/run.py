...
if args.variant == 'vanilla':
    ...
    my_gpt=model.GPT(mconf).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
elif args.variant == 'synthesizer':
    mconf.additive=True # transform to use SynthesizerAttention
    my_gpt = model.GPT(mconf).to(torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
...
if args.function == 'pretrain':
    ...
    tconf = trainer.TrainerConfig(max_epochs=650, batch_size=128, learning_rate=6e-3, lr_decay=True,
                                  ...
                                  num_workers=0)  # num_workers could be set to 4 when running in linux os
    my_trainer = trainer.Trainer(my_gpt, pretrain_dataset, None, tconf)
    my_trainer.train()
    torch.save(my_gpt.state_dict(), args.writing_params_path)
    ...
    if args.reading_params_path is None: # without a pretrained model
        tconf=trainer.TrainerConfig(max_epochs=200, batch_size=256, learning_rate=6e-4, lr_decay=True,
                                    warmup_tokens=512*20, final_tokens=200*len(finetune_dataset)*block_size,
                                    num_workers=0) # num_workers could be set to 4 when running in linux os
    else: # with a pretrained model
        tconf = trainer.TrainerConfig(max_epochs=100, batch_size=256, learning_rate=6e-4, lr_decay=True,
                                      warmup_tokens=512 * 20, final_tokens=200 * len(finetune_dataset) * block_size,
                                      num_workers=0) # num_workers could be set to 4 when running in linux os
        my_gpt.load_state_dict(torch.load(args.reading_params_path))

    my_trainer = trainer.Trainer(my_gpt, finetune_dataset, None, tconf)
    my_trainer.train()
    ...
    with open(args.outputs_path, 'w') as fout:
        ...
        for line in tqdm(open(args.eval_corpus_path)):
            ...
            pred = utils.sample(my_gpt, x, 32, sample=False)[0]
